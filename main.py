...
with st.sidebar:
    ...
    model_name, debug_mode = ui_sidebar.render_model_config()
    ui_sidebar.render_project_settings()
    ...
    st.divider()
    if model_ready:
        ui_sidebar.render_session_management(model, system_prompts)
    else:
        st.warning("Session management disabled until model is ready.")

    st.divider()
    if model_ready:
        ui_sidebar.render_artifact_manager(local_summarizer, system_prompts)
        ui_sidebar.render_snippet_manager()
    else:
        st.warning("Artifact management disabled until model is ready.")

    st.divider()
    ui_sidebar.render_provisional_context()

    st.divider()
    ui_sidebar.render_prompt_flow_selector()

    st.divider()
    ui_sidebar.render_instruction_profiles()

    st.divider()
    if model_ready:
        ui_sidebar.render_token_usage(model, sidebar_system_instruction)
    else:
        st.warning("Token usage cannot be calculated as model is not ready.")

    st.divider()
    ui_sidebar.render_system_controls()

# --- Main UI --- 
st.title("Lathe - Keeping the Vibe alive as you code.")

# --- Staged Revisions Review Panel ---
            ...
            with st.form(key="rejection_form"):
                ...
                if submit_rejection:
                    if not rejection_reason.strip():
                        ...
                        # 4. Construct and post the comprehensive failure/retry message
                        rejected_files_str = "\n".join(
